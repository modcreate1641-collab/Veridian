...
function Window.new(WindowConfig)
    local self = setmetatable({}, Window)
    return self
end

function Window:UpdateHubInfo(cfg)
...
function Window:UpdateTheme(newColor)
        ...
        if tab:IsA("ScrollingFrame") or tab:IsA("Frame") then
            for _, item in pairs(tab:GetChildren()) do
                ...
                elseif item:IsA("TextLabel") then
                    CreateTween(item, {TextColor3 = Color3.fromRGB(200, 200, 205)}, 0.5)
                end
            end
        end
    end
...
function Window:CreateTab(name, target, isAuto)
    return Tab.new(name, target, isAuto)
end

function Tab.new(name, target, isAuto)
...
function Tab:CreateLabel(cfg)
    return Label.new(cfg, self.TabPage)
end

function Tab:CreateButton(cfg)
    return Button.new(cfg, self.TabPage)
end

function Tab:CreateToggle(cfg)
    return Toggle.new(cfg, self.TabPage)
end

function Tab:CreateSlider(cfg)
    return Slider.new(cfg, self.TabPage)
end

function Tab:CreateDropdown(cfg)
    return Dropdown.new(cfg, self.TabPage)
end

function Label.new(cfg, TabPage)
...
function Button.new(cfg, TabPage)
...
function Toggle.new(cfg, TabPage)
    ...
    self.Callback = cfg.Callback
    self.CurrentValue = s
    btn.MouseButton1Click:Connect(function() self:Set() end)
    return self
end

function Toggle:Set(Value)
    if Value == nil then self.CurrentValue = not self.CurrentValue else self.CurrentValue = Value end
    CreateTween(self.SwitchBg, {BackgroundColor3 = self.CurrentValue and self.ColorOn or self.ColorOff}, 0.2)
    CreateTween(self.Knob, {Position = self.CurrentValue and UDim2.new(1, -18, 0.5, -8) or UDim2.new(0, 2, 0.5, -8)}, 0.2, Enum.EasingStyle.Back, Enum.EasingDirection.Out)
    pcall(self.Callback, self.CurrentValue)
end

function Slider.new(cfg, TabPage)
    ...
    self.Value = val
    self.Suffix = suffix
    self.CfgName = cfg.Name or "Slider"
    self.Callback = cfg.Callback
    local function updateSlider(customVal)
        if customVal then
            self.Value = math.clamp(customVal, self.Min, self.Max)
            local p = (self.Value - self.Min) / (self.Max - self.Min)
            CreateTween(self.Fill, {Size = UDim2.new(p, 0, 1, 0)}, 0.1)
        else
            local p = math.clamp((UserInputService:GetMouseLocation().X - self.Bar.AbsolutePosition.X) / self.Bar.AbsoluteSize.X, 0, 1)
            CreateTween(self.Fill, {Size = UDim2.new(p, 0, 1, 0)}, 0.1)
            self.Value = math.floor(self.Min + (p * (self.Max - self.Min)))
        end
        self.TextLabel.Text = self.CfgName .. " : " .. self.Value .. self.Suffix
    ...
    self.UpdateSliderFunc = updateSlider
    return self
end

function Slider:Set(Value)
    self.UpdateSliderFunc(Value)
end

function Dropdown.new(cfg, TabPage)
    ...
    self.IsOpen = open
    self.CfgName = cfg.Name or "Dropdown"
    self.Callback = cfg.Callback
    local function toggleDropdown()
        self.IsOpen = not self.IsOpen
        CreateTween(self.Instance, {Size = self.IsOpen and self.OpenedSize or self.ClosedSize}, 0.25, Enum.EasingStyle.Quart)
        CreateTween(self.Arrow, {Rotation = self.IsOpen and 180 or 0}, 0.25)
        if self.IsOpen then 
            self.DropScroll.Visible = true 
        else 
            task.delay(0.2, function() 
                if not self.IsOpen then self.DropScroll.Visible = false end 
            end) 
        end
    end
    mb.MouseButton1Click:Connect(toggleDropdown)
    self.ToggleDropdownFunc = toggleDropdown
    local function createOptions()
        for _, child in pairs(self.DropScroll:GetChildren()) do
            if child:IsA("TextButton") then child:Destroy() end
        end
        for i, opt in pairs(self.Options) do
            local o = Instance.new("TextButton", self.DropScroll)
            ...
            o.MouseButton1Click:Connect(function()
                self.MainButton.Text = "  " .. self.CfgName .. " : " .. tostring(opt)
                self.ToggleDropdownFunc()
                pcall(self.Callback, opt)
            end)
        end
    end
    self.CreateOptionsFunc = createOptions
    self.CreateOptionsFunc()
    return self
end

function Dropdown:Refresh(newOptions, _)
    self.Options = newOptions or {}
    local count = #self.Options
    self.DropScroll.CanvasSize = UDim2.new(0, 0, 0, count * self.ItemHeight)
    self.OpenedSize = UDim2.new(0.96, 0, 0, 42 + math.min(count * self.ItemHeight, self.MaxDisplayItems * self.ItemHeight))
    self.CreateOptionsFunc()
end

function Dropdown:Set(Value)
    self.MainButton.Text = "  " .. self.CfgName .. " : " .. tostring(Value)
    pcall(self.Callback, Value)
end

function VeridianLibrary:CreateWindow(WindowConfig)
    return Window.new(WindowConfig)
end
